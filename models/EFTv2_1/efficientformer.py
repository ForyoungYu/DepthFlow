...
from models.modules.blocks import MoTBlock, _make_divisible
...
class EfficientFormer(nn.Module):
    ...
    def __init__(self, layers, embed_dims=None,
        ...
        if self.fork_feat:
            ...
            for i_emb, i_layer in enumerate(self.out_indices):
                if i_emb == 0 and os.environ.get('FORK_LAST3', None):  # 获取'FORK_LAST3'环境变量值
                    layer = nn.Identity()
                else:
    ...
    def forward_tokens(self, x):
        ...
        H = 0
        W = 0
        for idx, block in enumerate(self.network):  # len(self.network) is 7
            if len(x.size()) == 4: # x的维度为四时将BCHW输出
                B, C, H, W = x.shape
            x = block(x)
            if self.fork_feat and idx in self.out_indices:  # self.out_indices is [0, 2, 4, 6]
